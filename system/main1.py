...
from flcore.servers.serveravg1 import FedAvg1
...
from flcore.servers.serveravg_test import FedAvg_test
...
def run(args):
    ...
    cluster_idx = {0:[1,4], 1:[0,2,3,5,6,7,8,9]}
    servers = []
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "MLR": # convex
            if "MNIST" in args.dataset:
                args.model = Mclr_Logistic(3*28*28, num_classes=args.num_classes).to(args.device)
            ...
            if "OrganAMNIST" in args.dataset:
                args.model = ppp(in_channels=1, num_classes=args.num_classes).to(args.device)
            elif "BloodMNIST" in args.dataset:
                args.model = bl(in_channels=3, num_classes=args.num_classes).to(args.device)
            ...
            #     args.model = DNN(3*28*28, 100, num_classes=args.num_classes).to(args.device)
            else:
        ...
        if args.algorithm == "FedAvg":
            ...
            for cluster_id, client_ids in cluster_idx.items():
                print(f"\n=== Initializing Server for Cluster {cluster_id} ===")
                server = FedAvg(args, i, client_ids)
                server.num_clients = len(cluster_idx)
                server.num_join_clients = int(server.num_clients * server.join_ratio)
                server.current_num_join_clients = len(cluster_idx)
                server.num_join_clients = len(cluster_idx)
                servers.append(server)
        
        elif args.algorithm == 'serveravg_self-attention':
            server = Selfatten(args,i)


       
        elif args.algorithm == 'FedAvg_PIFCA':
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedAvg_test(args, i)

...
from flcore.servers.serverala_clu import FedALA_clu
...
def run(args):
    ...
    cluster_idx = {0:[1,3,5], 1:[0,2,4,6,7,8,9]}
    servers = []
    ...
    for i in range(args.prev, args.times):
        ...
        if args.algorithm == "FedAvg":
        ...
        elif args.algorithm == 'FedAvg1':
            args.head = copy.deepcopy(args.model.fc)
            args.model.fc = nn.Identity()
            args.model = BaseHeadSplit(args.model, args.head)
            server = FedAvg1(args, i)
            ...
            for cluster_id, client_ids in cluster_idx.items():
                ...
                server = FedALA_clu(args, i, client_ids)
                server.num_clients = len(client_ids)
                server.num_join_clients = int(server.num_clients * server.join_ratio)
                server.current_num_join_clients = len(client_ids)
                server.num_join_clients = len(client_ids)

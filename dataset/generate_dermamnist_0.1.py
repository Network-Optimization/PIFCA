...
import random
import torch
from medmnist import DermaMNIST, INFO  # 修改为 DermaMNIST
import torchvision
import torchvision.transforms as transforms
from utils.dataset_utils import check, separate_data, split_data, save_file
...
np.random.seed(1)
num_clients = 10
dir_path = "DermaMNIST_100/"  # 修改目录名为 DermaMNIST_0.1
...
def generate_dataset(dir_path, num_clients, niid, balance, partition):
    ...
    # 加载训练集和测试集
    train_dataset = DermaMNIST(  # 修改为 DermaMNIST
        root=dir_path + "rawdata",
        split="train",
        download=True,
        transform=transform
    )
    test_dataset = DermaMNIST(  # 修改为 DermaMNIST
        root=dir_path + "rawdata",
        split="test",
        download=True,
        transform=transform
